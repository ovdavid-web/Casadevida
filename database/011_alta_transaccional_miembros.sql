-- Casa de Vida
-- Migración 011: alta transaccional de miembros.
--
-- Crea o reutiliza la identidad central, registra al miembro, conserva el
-- historial de vínculos y escribe auditoría dentro de una sola transacción.
...
create or replace function public.crear_miembro_con_persona(
    ...
    p_area_servicio text default null,
...
begin
    ...
    if p_correo is not null and char_length(btrim(p_correo)) > 100 then
        raise exception using
            errcode = '22023',
            message = 'El correo supera el máximo de 100 caracteres.';
    end if;

    if p_telefono is not null and char_length(btrim(p_telefono)) > 20 then
        raise exception using
            errcode = '22023',
            message = 'El teléfono supera el máximo de 20 caracteres.';
    end if;

    if p_direccion is not null and char_length(btrim(p_direccion)) > 200 then
        raise exception using
            errcode = '22023',
            message = 'La dirección supera el máximo de 200 caracteres.';
    end if;

    if p_area_servicio is not null and char_length(btrim(p_area_servicio)) > 50 then
        raise exception using
            errcode = '22023',
            message = 'El área de servicio supera el máximo de 50 caracteres.';
    end if;
    ...
    if found then
        if exists (
            ...
            where m.persona_id = v_persona.id
        ) then
            raise exception using
                errcode = '23505',
    ...
    values (
        ...
        nullif(btrim(p_telefono), ''),
        p_fecha_bautismo,
        coalesce(nullif(btrim(p_area_servicio), ''), 'Sin asignar'),
        case when p_activo then 'activo' else 'inactivo' end,
        p_activo,
        v_fecha_ingreso,
        nullif(btrim(p_direccion), ''),
    ...
    values (
        ...
        null,
        case
            when p_activo then null
            else 'Miembro registrado inicialmente como inactivo'
        end
    );
...
comment on function public.crear_miembro_con_persona(
    ...
    'Alta atómica de persona, miembro, vínculo y auditoría. Reutiliza identidades previas por RUT.';
